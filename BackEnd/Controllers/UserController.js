...
exports.requestPasswordReset = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Passcode sent to your email. It will expire in 20 minutes." });
...
exports.resetPassword = async (req, res) => {
    try {
        ...
        if (user.resetPasscode !== passcode || user.resetPasscodeExpires < new Date()) {
...
exports.getUserProfileById = async (req, res) => {
    try {
        ...
        const profile = await UserProfile.findById(req.params.id).populate('userId tags.tag questions answers');
